...
| 🎥 | **Scenes that never play** | A survey reported ten cutscene directors with nothing pointing at them - among them `UKAUKA_DEFEATED`, `BR_CORTEX_PIPE`, `CAVERN`, `EARTH_HUB` and `HUB2_TO_HUB3`. **That survey's method has now been shown to produce false positives and the list should not be trusted until it is redone.** `twinsdump refs <id>` returns nothing for object **972**, the Henchmania director in `gpa11` - a scene that demonstrably plays, watched on the rig and captured in a screenshot - and nothing for object **877**, Evil Crash, who certainly exists in `altdoc`. So "no references" from that tool does not mean unreferenced, and a director appearing on this list is not evidence it never runs. A usable version needs a detection method validated against scenes known to play, which is cheap now that several are confirmed. |
