...
| 🔒 | **Softlock after the Rusty Walrus chase** | Mashing jump through the cutscene skips the Brio/Tropy scene and strands Crash on the boss iceberg with no music. **Not a bug this mod introduced**: the scene is `HUB2_TO_HUB3_CUTSCENE_DIRECTOR` in `gpa11`, and no recipe in `mod/levels/` touches that file. `walrus.ops` is a different scene - gpa10's chase-start - and its own music loss is already fixed. The director is object 890, instance 40 at `(28.27, -64.6, 2.38)`, and **nothing in gpa11's scripts references it**, which puts it on the orphaned-director list below as well. Its skip path is `COM_HUB2_TO_HUB3_CUTSCENE_DIRECTOR_ACTIVATED` (script 5265). Next step is to read that script's skip branch and check whether it performs the level transition the full scene does - the same shape as the walrus fix, where the developers' unfinished skip omitted the music. |
