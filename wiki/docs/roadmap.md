...
|:-:|---|---|
| 🚩 | **Checkpoints in the long stretches** | `tools/checkpoint_survey.py` says **10 of the 59 substantial level files place no checkpoint of any kind** - among them the Earth hub, the 10th-dimension lab exterior, the Rockslide start and the Academy hub. That is fewer than the 22 counted before, because there are two kinds: the visible `act_CHECKPOINTCRATE` and an `act_INVISIBLE_CHECKPOINT_CRATE*` trigger volume that saves progress with nothing on screen. The boss arena has no crate but does have those, so it was never missing a checkpoint. **Adding an instance is no longer the blocker** - `rm2splice.insert()` does that, verified against the Twinsanity library reading 50 instances where it read 49. The blocker moved: **none of the 10 define a checkpoint object at all**, while all 82 files that define one also place one, so there is no level where an instance alone would do. Placing a checkpoint in any of the 10 means porting the object, its scripts, animations and OGIs in from another level file - a different and much larger job - and placements still have to be chosen in-game rather than guessed from coordinates. |
