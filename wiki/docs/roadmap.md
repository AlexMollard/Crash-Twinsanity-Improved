...
|:-:|---|---|
| 🚩 | **Checkpoints in the long stretches** | `tools/checkpoint_survey.py` says **10 of the 59 substantial level files place no checkpoint of any kind** - the Earth hub, the 10th-dimension lab exterior, the Rockslide start, the Academy hub among them. (Fewer than the 22 counted before: there are two kinds, the visible `act_CHECKPOINTCRATE` and an `act_INVISIBLE_CHECKPOINT_CRATE*` trigger volume, and the boss arena has the latter.) **Placing one is done and verified in game.** `tools/place_object.py` imports the object with its scripts, animations, OGIs, models and skins and places instances. The Earth hub, which defines no checkpoint object at all, takes beach's crate for **+87KB (+1.5%)**, and on the rig the crates render with their own textures, have collision, and break when Crash lands on one. **Whether a placed checkpoint then saves your progress is still unresolved**, and the obvious test does not settle it: three deaths on the same actor in huba respawned at (-60.9, 134.7), (-60.6, 135.6) and (-59.2, 112.9), so the respawn point moves on its own and a single before/after pair proves nothing. It needs repeated sampling, in a level whose respawn is stable, against a native crate as the control. `rig.kill()` is the tool for that. Placements still have to be chosen in-game rather than guessed from coordinates. |
