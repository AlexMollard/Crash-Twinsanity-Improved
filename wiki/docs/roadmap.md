...
|:-:|---|---|
| 🚩 | **Checkpoints in the long stretches** | `tools/checkpoint_survey.py` says **10 of the 59 substantial level files place no checkpoint of any kind** - the Earth hub, the 10th-dimension lab exterior, the Rockslide start, the Academy hub among them. (Fewer than the 22 counted before: there are two kinds, the visible `act_CHECKPOINTCRATE` and an `act_INVISIBLE_CHECKPOINT_CRATE*` trigger volume, and the boss arena has the latter.) **The tooling is done and verified in game.** `tools/place_object.py` imports the object with its scripts, animations, OGIs, models and skins and places instances of it. The Earth hub, which defines no checkpoint object at all, takes beach's crate plus two placed crates for **+87KB (+1.5%)**, and on the rig the crates **render with their own models and textures, have collision, and break when Crash lands on one**. Still open: whether a placed checkpoint actually saves progress (needs a death and a respawn), and choosing placements in-game rather than guessing them from coordinates. |
