...
|:-:|---|---|
| 🚩 | **Checkpoints in the long stretches** | `tools/checkpoint_survey.py` says **10 of the 59 substantial level files place no checkpoint of any kind** - the Earth hub, the 10th-dimension lab exterior, the Rockslide start, the Academy hub among them. (Fewer than the 22 counted before: there are two kinds, the visible `act_CHECKPOINTCRATE` and an `act_INVISIBLE_CHECKPOINT_CRATE*` trigger volume that saves progress with nothing on screen, and the boss arena has the latter.) **Both halves of the tooling now exist.** `rm2splice.insert()` adds the instance, and the editor library's `MergeObject()` ports the object with its scripts, animations and OGIs - object IDs are consistent across level files, so 266 is the checkpoint crate everywhere. Demonstrated end to end: the Earth hub, which defines no checkpoint object at all, takes beach's crate for **+86KB (+1.5%)** and re-saves byte-identical with a placed instance. What is left is not tooling - it is choosing placements in-game rather than guessing them from coordinates, and confirming on the rig that the imported crate actually works in play. |
