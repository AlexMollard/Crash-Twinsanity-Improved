...
**How far to trust that 871 is a caution in its own right.** The same route returns 299 in the control level, and object 299 does exist there - it is a wumpa tree, which is not an obvious thing to be running a focus search. And 871 is defined in *none* of the three files that make up Bandicoot Pursuit (`altdoc`, `altdoc_b`, `altdoc_c`), nor in the shared `Startup\Default.rm2`, although ids in that level run up to 1131. So the route returns ids that exist sometimes and ids that do not exist other times, which is not yet good enough to build on.

What is solid: the two looping agents are one and the same object, and it is not 877. What is not: which object it actually is.
