...
- *"the control resolved to a wumpa tree"* - an artifact of naming an id from one level's table. Agents with id **299 run in `altdoc`, where 299 is not in the object list at all**; it appears only in `hubd`'s. So ids are not per-file indices, and a name looked up in a single level's partial listing can be an unrelated object that happens to share the number.

So 871 is a real object id, and what it names is still open - it needs a name source that is not one level's partial table.
