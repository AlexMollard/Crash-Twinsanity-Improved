...
|:-:|---|---|
| 🧭 | **Evil Crash runs in circles (Bandicoot Pursuit)** | The famous PAL one. His run heading sits about 20° off the route and he orbits a node of radius 1.0 instead of entering it. Now known to be engine-side: his move script has no facing condition and no turn command at all - it sets a focus position and the engine steers him - so there is no script tolerance to widen. **Reproducing it again is the current blocker**: a plain warp into `altdoc` does not start the chase (Evil Crash never leaves his spawn), so it needs story progress and probably Cortex present, the way the Psychetron scenes needed `progress=N`. |
