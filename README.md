...
| 🐌 | **Slow menus** | [Documented as a PAL trait](https://beyondtwinsanity.com/evolution/versions/), and measured: on the retail disc a menu button takes about **0.82 s** to produce any visible response. The modded build's figure is still missing - the front end animates constantly and the two builds boot at different speeds, so the harness (`tools/rig/menu_time.py`) keeps landing in the attract demo. |
