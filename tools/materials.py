...
CRATE_NAMES = re.compile(rb"^(AKU)?CRATES?_", re.I)
PARAMS = {23: 12, 26: 20, 16: 4, 17: 4}

def materials(buf):
    """Yield (name, [(settings_offset, ablending, fba)]) for every material in an RM2, verifying the layout."""
    try: base, _ = rm2splice.locate(buf, [11, 1])
    except (StopIteration, struct.error): return
    _, _, recs = rm2splice._records(buf, base)
    for off, size, _ in recs:
        ...
        for _ in range(count):
            ...
            p += 24 + 6 + 4 + 48 + 8
        if p == end: yield name, shaders

def crate_shadow_offsets(buf):
    """Offsets of the FBA byte of every opaque crate shader that has it off."""
    return [s + 24 for name, shaders in materials(buf) if CRATE_NAMES.match(name)
...
if __name__ == "__main__":
    for path in sys.argv[1:]:
        ...
        crates = [(n, sh) for n, sh in mats if CRATE_NAMES.match(n)]
